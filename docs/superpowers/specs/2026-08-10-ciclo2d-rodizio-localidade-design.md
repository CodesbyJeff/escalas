# Ciclo 2d — Rodízio por Localidade — Design

**Data:** 2026-08-10
**Contexto:** Última perna declarada do Ciclo 2. O balanço ESAON (01/07) define o ciclo como *"geração em bloco 24h×72h **+ rodízio de guarnições/locais do GBSA** por equidade"*. A geração em bloco (2a), a elegibilidade (2b) e o motor de preenchimento (2c) foram entregues; o rodízio nunca virou fatia. Hoje o motor equilibra **quantos** serviços cada militar tirou, mas não **onde** — no GBSA, onde a guarnição é uma praia, isso deixa o mesmo militar preso ao mesmo posto.

**Fonte de domínio:** escala real do BSA de 08 a 11/08/2026 (4 dias consecutivos, publicada pelo escalante do BSA), lida nesta sessão.

## O que a escala real mostrou

- **Ciclo 24×48, três turmas.** O serviço ordinário de cada militar cai de 3 em 3 dias: 08 e 11 são a mesma turma; 09 e 12 outra; 10 e 13 a terceira. Tudo que aparece nos dias intermediários vem marcado `(D.O.)`.
...
- **Critério de equidade passa a ser por localidade, com o total como desempate.** Primeiro quem tirou menos serviços **naquela localidade**; depois quem tirou menos serviços no total.
...
- **Ligado por layout, não por execução.** Quartel é fixo por definição operacional ("os quartéis são fixos em incêndio, resgate, salvamento"); GBSA rodizia sempre. É característica permanente da lotação, então mora no `TemplateLotacao`. Layout sem a flag mantém o comportamento atual **byte por byte**.
...
- **Turma (Alfa/Bravo/Charlie) não vira modelo.** Ela emerge de `descanso_horas = 48` sobre turno de praia 07→17: quem serviu no dia 8 só volta a ficar elegível a tempo do dia 11. Foi o que o dado real mostrou, inclusive com a folga que uma turma rígida não teria.
...
`TemplateLotacao` ganha uma coluna:

```prisma
rodizia_localidade Boolean @default(false)
```

Migration `0012_layout_rodizio_localidade`. Default `false` preserva todos os layouts existentes; só o "Padrão (mapa de força)" do GBSA será marcado.

### Serviço — `apps/backend/src/services/preenchimento.service.ts`
- **Lê a flag:** `escala.template_id` → `TemplateLotacao.rodizia_localidade`. `template_id` é nullable (`ON DELETE SET NULL`); nulo ⇒ `false`.
...
- **A fonte não muda.** Mesmas escalas, mesmo recorte temporal, mesma regra de status. Só o agrupamento é novo.
...
- `PlanoInput` ganha `contagemLocalInicial: Map<number, Map<string, number>>` e `rodiziaLocalidade: boolean`.
...
cands.sort((a, b) =>
  ...
  (input.rodiziaLocalidade ? a.contagemLocal - b.contagemLocal : 0) ||
  a.contagem - b.contagem ||
  a.id - b.id);
```

Com `rodiziaLocalidade = false` o termo é constante zero e a ordenação é idêntica à de hoje.

- **Atualização incremental:** ao atribuir, além de `contagem`, incrementa `contagemLocal` do militar escolhido naquela localidade — o rodízio precisa valer **dentro da mesma rodada**, não só contra o histórico. É o que faz o motor espalhar as praias ao longo do mês numa única execução.
- **`motivo`:** quando o rodízio está ligado, a explicação abre com a localidade — `"menos serviços em Praia de Ponta Negra (1) · menos serviços (11) · descansado · patente ok"`. Exibe a **atividade original** da guarnição, não a chave normalizada: a chave serve para contar, o motivo para o escalante ler. Desligado, a string permanece exatamente a de hoje.

### API / DTO
**O contrato do motor não muda.** A flag vem do layout, não do corpo da requisição: `preenchimentoInputSchema` (`data_ini`, `data_fim`, `descanso_horas?`) e `PreenchimentoSugestaoDTO` ficam intactos, e os dois endpoints (`POST /escalas/:id/sugerir-preenchimento` e `/aplicar-preenchimento`) mantêm assinatura e resposta.

**O CRUD de layouts muda:** `POST`/`PUT /api/v1/templates/:id` passa a aceitar e devolver `rodizia_localidade`; o schema Zod do layout ganha o campo booleano opcional com default `false`.

### Web — `apps/web`
- **Editor de layout:** um `Switch` do Mantine — *"Rodiziar militares entre as localidades"* — com texto de apoio explicando que o motor vai preferir quem tirou menos serviços naquela guarnição. Fica junto dos demais campos do layout.
- **Cartão de preenchimento automático:** sem campo novo. A coluna *Motivo* da tabela de preview já mostra a explicação, que passa a citar a localidade quando o rodízio está ativo.

## Escopo / não-escopo

**Em escopo:** coluna no layout + migration, contagem por localidade no serviço, critério no núcleo puro, campo no CRUD de layouts, switch no editor de layout, testes.

**Fora:**
- Conceito de turma/equipe (Alfa/Bravo/Charlie) no modelo — emerge do descanso.
...
**Núcleo puro (Vitest):**
- Com rodízio ligado, entre dois militares com o mesmo total, vence o que tirou menos naquela localidade.
- Com rodízio ligado, entre dois militares com a mesma contagem local, o total desempata.
- Rodízio ligado espalha as localidades **dentro de uma única rodada** (militar escalado em Ponta Negra no dia 1 não é o primeiro candidato a Ponta Negra no dia 4, tendo par disponível).
- Rodízio desligado produz saída **idêntica** à de hoje — teste de regressão sobre um caso já existente da suíte.
- Conflito de turno continua hard mesmo com rodízio ligado.
...
- Layout com `rodizia_localidade = false` (e escala com `template_id` nulo) não altera comportamento.
- `aplicar` continua gravando só em vaga aberta e auditando.

**Web:**
- O switch persiste no layout (salvar e reabrir).
- O preview exibe o motivo com a localidade quando o rodízio está ativo.
...
- **Pool pequeno numa localidade específica.** Onde a função exige qualificação concentrada (MERGULHADOR, SUP_MERGULHO), o critério de localidade pode empurrar para fora quem de fato deve estar ali. Como patente é critério **anterior** ao rodízio no ranqueio, a elegibilidade continua vencendo — mas vale observar no uso real.
- **Primeira execução sem histórico.** Numa lotação sem escala anterior publicada, toda contagem local começa zerada e o desempate cai no total e depois no `militar_id`. Comportamento correto, mas a primeira escala do GBSA sairá menos "rodiziada" que as seguintes.
